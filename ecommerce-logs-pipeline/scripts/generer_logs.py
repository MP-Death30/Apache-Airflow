...
Usage : python3 generer_logs.py <date YYYY-MM-DD> <nb_lignes> <fichier_sortie>
...
# Correction syntaxique des chaînes tronquées
USER_AGENTS = [
...
def generer_log_line(date_str: str) -> str:
    ...
    methode, url, status, taille = random.choices(
        URLS,
        weights=[u[2] for u in URLS], # Pondération par status code
        k=1
    )[0]
    ...
    # Format Combined Log Format : IP - - [timestamp] "METHOD URL HTTP/1.1" status taille "referrer" "user agent"
    return (
...
def main():
    if len(sys.argv) != 4:
        # Correction syntaxique de la chaîne tronquée
